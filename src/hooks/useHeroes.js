/**
 * useHeroes.js
 * Custom React hook — loads all heroes once on app startup and exposes them globally.
 * Think of this like a Start() method in Unity that populates a static data store.
 */

import { useState, useEffect } from 'react';
import { loadAllHeroes } from '../utils/heroLoader';

export function useHeroes() {
  const [heroes, setHeroes] = useState([]);       // All loaded hero objects
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const data = await loadAllHeroes();
        if (!cancelled) {
          setHeroes(data);
          setLoading(false);
        }
