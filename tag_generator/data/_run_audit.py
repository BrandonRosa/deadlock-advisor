...
The Normalized column is already authored by `_normalize.py`, which pre-blends
`(adds + 0.5 × relies)` of the comparative raws BEFORE normalizing, writes the
single Normalized into the first `adds` row per (item, tag), and leaves `—` on
relies / duplicate-adds rows. So this audit just sums the Normalized values per
tag per item (the `—` rows are skipped at parse) and compares to each item's
current `data/items/<key>.json` → values.playstyle_score.
    ...
    for chunk in re.split(r'\n##\s+', body)[1:]:
        ...
        if ct:
            for line in ct.group(1).splitlines():
                ...
                # In Round-10 pipeline, only the first adds row per tag has a numeric Normalized;
                # later adds/relies rows for the same tag show "—" and are skipped by parse_float.
...
def blend(rows):
    """Round-10 pipeline: blending happens in _normalize.py BEFORE the Normalized column
    is written. Here we just sum per-tag Normalized values across whatever numeric rows
    survived parsing (relies rows and duplicate adds rows for a tag carry "—" and are
    skipped at parse). Duplicate numeric rows for the same tag (rare) sum together."""
    by_tag = {}
    for tag, norm, _mode in rows:
        by_tag[tag] = by_tag.get(tag, 0.0) + norm
    return {t: round(v, 2) for t, v in by_tag.items()}
...
def main():
    ...
    a('')
    a('**Blending convention** (Round 10+): Single Normalized per (item, tag) authored by `_normalize.py` from `(adds + 0.5 × relies)` BEFORE normalization. The audit reads it directly — no further blending.')
    a('')
